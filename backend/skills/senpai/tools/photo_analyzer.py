...
提取照片 EXIF 信息（拍摄时间、地点），辅助构建组内时间线、
合照名场面和白板现场的上下文。
...
def get_exif_data(image_path: str) -> dict:
    ...
    try:
        ...
        return {"file": os.path.basename(image_path), "error": str(exc)}
    ...
    with open(args.output, "w", encoding="utf-8") as f:
        ...
        if undated_photos:
            ...
            for photo in undated_photos:
                f.write(f"- {photo.get('file', photo.get('path', 'unknown'))}\n")
