...
import { getSkillBody } from '../services/skills';
...
export class ContextManager {
  ...
  private skillPersonaCache: Map<string, string | null> = new Map();

  /**
   * Return the system prompt combining ghost persona, skill persona, and memory context.
   */
  async getSystemPrompt(grave: Grave): Promise<string> {
    const skillType = (grave as any).skillType as string | undefined;
    let skillPersona: string | null = null;

    if (skillType) {
      if (this.skillPersonaCache.has(skillType)) {
        skillPersona = this.skillPersonaCache.get(skillType)!;
      } else {
        skillPersona = await getSkillBody(skillType);
        this.skillPersonaCache.set(skillType, skillPersona);
      }
    }
